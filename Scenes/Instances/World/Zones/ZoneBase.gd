...
@export var max_entities: int: int = 25 # how many entites are allowed to spawn?
...
@rpc(any_peer, call_local) func SyncAllData():
	if(!EntitesState.is_empty() and get_tree().get_unique_id() != 1):
...
func LateSpawnEntity(entity):
	...
	var EntityToSpawn = Entityspawnid.instantiate()
